...
SELECT
    ...
    l.created_by_id,
    l.created_at,
    -- If l.created_at is TIMESTAMPTZ, this yields a TIMESTAMP WITHOUT TIME ZONE in NY time.
...
LEFT JOIN public.users AS u ON l.owner_id::text = u._id::text
LEFT JOIN public.orgs  AS o ON u.org_id::text   = o._id::text
WHERE COALESCE(u.clerk_dev_issuer, FALSE) IS NOT TRUE;  -- exclude dev/test, keep NULLs
